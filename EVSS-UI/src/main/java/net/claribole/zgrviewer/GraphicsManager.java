/*
 * FILE: GraphicsManager.java DATE OF CREATION: Mon Nov 27 08:30:31 2006 Copyright (c) INRIA,
 * 2006-2013. All Rights Reserved Licensed under the GNU LGPL. For full terms see the file COPYING.
 *
...
public class GraphicsManager implements ComponentListener, CameraListener, Java2DPainter {
  ...
  VirtualSpace rSpace; // virtual space containing rectangle representing region seen through main
                       // camera (used in overview)
  static final String mainSpaceName = "graphSpace";
  static final String menuSpace = "menuSpace";
  /*
   * name of the VTM virtual space holding the rectangle delimiting the region seen by main view in
   * radar view
   */
  ...
  /*
   * Some versions of GraphViz generate a rectangle representing the graph's bounding box. We don't
   * want this rectangle neither to be highlighted nor sensitive to zoom clicks, so we try to find
   * it after the SVG has been parsed, and we give it special treatment. null if no bounding box was
   * found in the SVG.
   */
  ...
  /*
   * logical structure available only if a recent version of GraphViz was used to generate the SVG
   * file (e.g. 2.13, maybe earlier)
   */
  ...
  Vector createZVTMelements(boolean applet) {
    ...
    SIRectangle seg2;
    observedRegion =
        new VRectangle(0, 0, 0, 10, 10, ConfigManager.OBSERVED_REGION_COLOR,
  ...
  void createFrameView(Vector cameras, String vt, JMenuBar jmb) {
    mainView =
        vsm.addFrameView(cameras, ConfigManager.MAIN_TITLE, vt, ConfigManager.mainViewW,
            ConfigManager.mainViewH, true, false, true, jmb);
  ...
  void parameterizeView(BaseEventHandler eh) {
    ...
    // XXX: TBD: refactor event handler code taking advantage of new one handler per layer
    // functionality
  ...
  /*
   * Starting at version ? (somewhere between 1.16 and 2.8), GraphViz programs generate a polygon
   * that bounds the entire graph. Attempt to identify it so that when clicking in what appears to
   * be empty space (but is actually the bounding box), the view does not get unzoomed. Also prevent
   * border highlighting when the cursor enters this bounding box.
   */
  void seekBoundingBox() {
    ...
    for (int i = 0; i < v.size(); i++) {
      if (v.elementAt(i) instanceof VRectangleOr) {
        ...
        if (largestRectangle == null || bigger(r, largestRectangle)) {
          // first rectangle encountered in the list, or compare this rectangle to biggest rectangle
          // at this time
          largestRectangle = r;
          lri = i;
        }
    ...
    // XXX: disabled check for objects lower in the stack as their seems to be some unidentified
    // object that messes around
    // in some versions of GraphViz such as 2.14 - this workaround should be fairly safe until we
  ...
  boolean containedIn(Glyph g, VRectangle r) {
    if (g instanceof DPath || g instanceof VText) {
      return true;// don't take edges and text into accout, would be too costly (and would require
                  // one repaint for text)
    } else {// just get geometrical center for other glyphs ; this is an approximation, but it
            // should work
  ...
  public void getHigherView() {
    ...
    Animation a =
        animator.getAnimationFactory().createCameraAltAnim(ConfigManager.ANIM_MOVE_LENGTH, c, alt,
  ...
  public void translateView(short direction) {
    ...
    Animation a =
        animator.getAnimationFactory().createCameraTranslation(ConfigManager.ANIM_MOVE_LENGTH, c,
  ...
  public void goTo(Location l) {
    ...
    Animation at =
        animator.getAnimationFactory().createCameraTranslation(ConfigManager.ANIM_MOVE_LENGTH,
            mSpace.getCamera(0), (Point2D.Double) animParams.elementAt(1), true,
            SlowInSlowOutInterpolator.getInstance(), null);
    Animation aa =
        animator.getAnimationFactory().createCameraAltAnim(ConfigManager.ANIM_MOVE_LENGTH,
            mSpace.getCamera(0), (Double) animParams.elementAt(0), true,
            SlowInSlowOutInterpolator.getInstance(), null);
  ...
  public void showRadarView(boolean b) {
    if (b) {
      if (rView == null) {
        ...
        vsm.addFrameView(cameras, RADAR_VIEW_NAME, View.STD_VIEW, ConfigManager.rdW,
            ConfigManager.rdH, true);
        ...
        rView.setLocation(0, 50); // 12.6 // added
        // same event handler handling all layers for now
        // XXX: TBD: refactor event handler code taking advantage of new one-handler-per-layer
        // functionality
  ...
  public void zoomInPhase1(int x, int y) {
    ...
    Animation a =
        animator.getAnimationFactory().createLensMagAnim(LENS_ANIM_TIME, (FixedSizeLens) lens,
  ...
  public void zoomInPhase2(double mx, double my) {
    ...
    if (cameraAbsAlt + deltAlt.floatValue() > FLOOR_ALTITUDE) {
      ...
      Animation at =
          animator.getAnimationFactory().createCameraTranslation(LENS_ANIM_TIME, mainCamera,
              new Point2D.Double(c2x - mainCamera.vx, c2y - mainCamera.vy), true,
              IdentityInterpolator.getInstance(), null);
      Animation aa =
          animator.getAnimationFactory().createCameraAltAnim(LENS_ANIM_TIME, mainCamera, deltAlt,
      ...
      Animation at =
          animator.getAnimationFactory().createCameraTranslation(LENS_ANIM_TIME, mainCamera,
              new Point2D.Double((c2x - mainCamera.vx) * ratio, (c2y - mainCamera.vy) * ratio),
              true, IdentityInterpolator.getInstance(), null);
      Animation aa =
          animator.getAnimationFactory().createCameraAltAnim(LENS_ANIM_TIME, mainCamera,
  ...
  public void zoomOutPhase1(int x, int y, double mx, double my) {
    ...
    Animation aa =
        animator.getAnimationFactory().createCameraAltAnim(LENS_ANIM_TIME, mainCamera,
            new Double(cameraAbsAlt * (MAG_FACTOR - 1)), true, IdentityInterpolator.getInstance(),
            null);
    animator.startAnimation(al, false);
    animator.startAnimation(at, false);
    animator.startAnimation(aa, false);
    ...
    synchronized (lens) {
      ...
      if (nmf <= MAX_MAG_FACTOR && nmf > 1.0f) {
        ...
        if (zooming == GraphicsManager.ZOOMOUT_LENS) {
          /*
           * if unzooming, we want to keep the focus point stable, and unzoom the context this means
           * that camera altitude must be adjusted to keep altitude + lens mag factor constant in
           * the lens focus region. The camera must also be translated to keep the same region of
           * the virtual space under the focus region
           */
          double a1 = mainCamera.getAltitude();
          lens.setMaximumMagnification((float) nmf, true);
          /*
           * explanation for the altitude offset computation: the projected size of an object in the
           * focus region (in lens space) should remain the same before and after the change of
           * magnification factor. The size of an object is a function of the projection coefficient
           * (see any Glyph.projectForLens() method). This means that the following equation must be
           * true, where F is the camera's focal distance, a1 is the camera's altitude before the
           * move and a2 is the camera altitude after the move: MAG_FACTOR * F / (F + a1) =
           * MAG_FACTOR + magOffset * F / (F + a2) From this we can get the altitude difference (a2
           * - a1)
           */
          mainCamera.altitudeOffset((a1 + mainCamera.getFocal()) * magOffset
              / (MAG_FACTOR - magOffset));
          /*
           * explanation for the X offset computation: the position in X of an object in the focus
           * region (lens space) should remain the same before and after the change of magnification
           * factor. This means that the following equation must be true (taken by simplifying
           * pc[i].lcx computation in a projectForLens() method): (vx-(lensx1))*coef1 =
           * (vx-(lensx2))*coef2 -- coef1 is actually MAG_FACTOR * F/(F+a1) -- coef2 is actually
           * (MAG_FACTOR + magOffset) * F/(F+a2) -- lensx1 is actually camera.vx1 + ((F+a1)/F) *
           * lens.lx -- lensx2 is actually camera.vx2 + ((F+a2)/F) * lens.lx Given that (MAG_FACTOR
           * + magOffset) / (F+a2) = MAG_FACTOR / (F+a1) we eventually have: Xoffset = (a1 - a2) / F
           * * lens.lx (lens.lx being the position of the lens's center in JPanel coordinates w.r.t
           * the view's center - see Lens.java)
           */
          ...
          Animation a =
              animator.getAnimationFactory().createLensMagAnim(WHEEL_ANIM_TIME,
                  (FixedSizeLens) lens, new Float(magOffset), true,
                  IdentityInterpolator.getInstance(), null);
          animator.startAnimation(a, false);
        }
      }
  ...
  void createDM(int x, int y, PortalListener pl) {
    dmPortal =
        new DraggableCameraPortal(x, y, GraphicsManager.DM_PORTAL_WIDTH,
    ...
    dmCamera
        .setAltitude(((mainCamera.getAltitude() + mainCamera.getFocal()) / (DEFAULT_MAG_FACTOR) - mainCamera
            .getFocal()));
    updateMagWindow();
    int w =
        (int) Math.round(magWindow.getWidth() * mainCamera.getFocal()
            / ((float) (mainCamera.getFocal() + mainCamera.getAltitude())));
    int h =
        (int) Math.round(magWindow.getHeight() * mainCamera.getFocal()
            / ((float) (mainCamera.getFocal() + mainCamera.getAltitude())));
    dmPortal.sizeTo(w, h);
    ...
    Animation as =
        animator.getAnimationFactory().createPortalSizeAnim(GraphicsManager.DM_PORTAL_ANIM_TIME,
            dmPortal, GraphicsManager.DM_PORTAL_WIDTH - w, GraphicsManager.DM_PORTAL_HEIGHT - h,
            true, IdentityInterpolator.getInstance(), null);
    Animation at =
        animator.getAnimationFactory().createPortalTranslation(
            GraphicsManager.DM_PORTAL_ANIM_TIME,
            dmPortal,
            new Point(GraphicsManager.DM_PORTAL_INITIAL_X_OFFSET - w / 2,
                GraphicsManager.DM_PORTAL_INITIAL_Y_OFFSET - h / 2), true,
            IdentityInterpolator.getInstance(), null);
  ...
  public void paint(Graphics2D g2d, int viewWidth, int viewHeight) {
    if (paintLinks) {
      ...
      g2d.drawLine(magWindowX + magWindowW, magWindowY - magWindowH, dmPortal.x + dmPortal.w,
          dmPortal.y);
      g2d.drawLine(magWindowX - magWindowW, magWindowY + magWindowH, dmPortal.x, dmPortal.y
          + dmPortal.h);
      g2d.drawLine(magWindowX + magWindowW, magWindowY + magWindowH, dmPortal.x + dmPortal.w,
          dmPortal.y + dmPortal.h);
    }
  ...
  void search(String s, int direction) {
    if (s.length() > 0) {
      if (!s.toLowerCase().equals(lastSearchedString)) {// searching a new string - reinitialize
                                                        // everything
        resetSearch(s);
        Glyph[] gl = mSpace.getVisibleGlyphsList();
        for (Glyph element : gl) {
          if (element instanceof VText) {
            if ((((VText) element).getText() != null)
                && (((VText) element).getText().toLowerCase().indexOf(lastSearchedString) != -1)) {
      ...
      if (matchSize > 0) {
        ...
        if (matchSize > 1) {
          zapp.setStatusBarText(AppletUtils.rankString(searchIndex + 1) + " of " + matchSize
              + " matches");
        } else {
        ...
        mainView.centerOnGlyph(g, mSpace.getCamera(0), ConfigManager.ANIM_MOVE_LENGTH, true,
  ...
  void assignFontToGraph() {
    Font f =
        fr.inria.zvtm.widgets.FontDialog.getFontDialog((JFrame) mainView.getFrame(),
            ConfigManager.defaultFont);
    if (f != null) {
  ...
  public void componentResized(ComponentEvent e) {
    if (e.getSource() == mainView.getFrame()) {
      updatePanelSize();
      // update rectangle showing observed region in radar view when main view's aspect ratio
      // changes
      cameraMoved(null, null, 0);
  ...
  void buildLogicalStructure() {
    // clone the structure as we are about to remove elements from it for convenience (it is
     ...
     * (which should have remained Metadata instances). Convert these old owners into LElem
     * instances (as this is what ZGRViewer now expects to get when calling Glyph.getOwner())
     */
  ...
  public LEdge addEdge(LNode sn, LNode en, String title, boolean directed) {
    ...
    Area edgeseg =
        new Area(st.createStrokedShape(new Line2D.Double(sns.vx, sns.vy, ens.vx, ens.vy)));
    ...
    Point2D.Double cpqd1 =
        new Point2D.Double((3 * spc.getCenterX() + epc.getCenterX()) / 4d,
            (3 * spc.getCenterY() + epc.getCenterY()) / 4d);
    // point of junction between both quadratic curves
    Point2D.Double jp =
        new Point2D.Double((spc.getCenterX() + epc.getCenterX()) / 2d,
            (spc.getCenterY() + epc.getCenterY()) / 2d);
    // control point of 2nd quadratic curve
    Point2D.Double cpqd2 =
        new Point2D.Double((spc.getCenterX() + 3 * epc.getCenterX()) / 4d,
    ...
    double theta =
        Math.atan2(epc.getCenterY() - spc.getCenterY(), epc.getCenterX() - spc.getCenterX());
    final VShape arrowHead =
        new VShape(epc.getCenterX(), epc.getCenterY(), 0, ens.getSize() / 10.0, TRIANGLE_VERTICES,
            Color.BLACK, theta);
  ...
  public void highlightElement(Glyph g, Camera cam, VCursor cursor, boolean highlight,
      int moreLess, boolean fixNode, int dir) {
    Object o = null;
  ...
  synchronized void highlightNode(LNode n, boolean highlight, short dir, int levelCnt) {
    if (highlight) {
      ...
      for (int i = 0; i < n.edges.length; i++) {
        ...
        for (Glyph glyph : n.edges[i].glyphs) {
          g = glyph;
          /*
           * prevent elements from being processed more than once this can happen when a node links
           * to itself, and this makes the highlighting mechanism think that the arc's original
  ...
  synchronized void highlightEdge(LEdge e, boolean highlight) {
    if (highlight) {
      ...
      // highlight tail
      for (Glyph glyph : e.tail.glyphs) {
        g = glyph;
        /*
         * prevent elements from being processed more than once this can happen when a node links to
         * itself, and this makes the highlighting mechanism think that the arc's original color is
         * the selection color
         */
        if (highlightedNodes.contains(g)) {
          continue;
  ...
  void startBringAndGo(Glyph g) {
    ...
    for (LEdge arc : arcs) {
      ...
      // process all others
      LNode otherEnd = arc.getOtherEnd(n);
      ClosedShape otherEndShape = otherEnd.getShape();
      double d =
          Math.sqrt((otherEndShape.vx - thisEndShape.vx) * (otherEndShape.vx - thisEndShape.vx)
              + (otherEndShape.vy - thisEndShape.vy) * (otherEndShape.vy - thisEndShape.vy));
      // use this end's shape size when getting ring because we're rescaling
      // brought nodes to match the size of the current node
      Ring ring =
          rm.getRing(
              Math.atan2(otherEndShape.vy - thisEndShape.vy, otherEndShape.vx - thisEndShape.vx),
    ...
    for (LEdge arc : arcs) {
      ...
      LNode otherEnd = arc.getOtherEnd(n);
      ClosedShape otherEndShape = otherEnd.getShape();
      bring(arc, otherEnd, thisEndShape.vx, thisEndShape.vy, otherEndShape.vx, otherEndShape.vy,
          thisEndShape.getSize(), node2broughtPosition);
    }
    ...
    for (int i = 0; i < elementsToFade.size(); i++) {
      try {
        ...
        Animation a =
            animator.getAnimationFactory().createTranslucencyAnim(BRING_ANIM_DURATION, t,
                FADED_ELEMENTS_TRANSLUCENCY, false, IdentityInterpolator.getInstance(), null);
  ...
  void endBringAndGo(Glyph g) {
    ...
    for (int i = 0; i < elementsToFade.size(); i++) {
      try {
        // XXX:TBW animate only if total number of objects is not too high
        Animation a =
            animator.getAnimationFactory().createTranslucencyAnim(BRING_ANIM_DURATION,
                (Translucent) elementsToFade.elementAt(i), alphaOfElementsToFade[i], false,
                IdentityInterpolator.getInstance(), null);
        animator.startAnimation(a, true);
      } catch (ClassCastException e) {
        e.printStackTrace();
    ...
    if (newCameraLocation != null) {
      Animation a =
          animator.getAnimationFactory().createCameraTranslation(BRING_ANIM_DURATION, mainCamera,
              newCameraLocation, false, SlowInSlowOutInterpolator.getInstance(), null);
      animator.startAnimation(a, true);
    }
  }

  void bring(LEdge arc, LNode node, double sx, double sy, double ex, double ey, double size,
      Hashtable node2broughtPosition) {
    ...
    if (size < nodeShape.getSize()) {
      Animation szAnim =
          animator.getAnimationFactory().createGlyphSizeAnim(BRING_ANIM_DURATION, nodeShape, size,
    ...
    Point2D.Double translation =
        new Point2D.Double(bposition.x - nodeShape.vx, bposition.y - nodeShape.vy);
    Glyph[] glyphs = node.getGlyphs();
    for (Glyph glyph : glyphs) {
      elementsToFade.remove(glyph);
      Animation posAnim =
          animator.getAnimationFactory().createGlyphTranslation(BRING_ANIM_DURATION, glyph,
              translation, true, SlowInSlowOutInterpolator.getInstance(), null);
    ...
    Animation a =
        animator.getAnimationFactory().createPathAnim(BRING_ANIM_DURATION, spline, flatCoords,
            false, SlowInSlowOutInterpolator.getInstance(), null);
    animator.startAnimation(a, true);
    ...
    for (Glyph glyph : glyphs) {
      if (glyph != spline) {
        ...
        if (glyph instanceof VText) {
          ...
          a =
              animator.getAnimationFactory().createGlyphTranslation(BRING_ANIM_DURATION, glyph,
                  new Point2D.Double(bposition.x - sx, bposition.y - sy), true,
                  SlowInSlowOutInterpolator.getInstance(), null);
          animator.startAnimation(a, true);
        } else {
          // probably a tail or head decoration ; just hide it for now, we don't know how to
    ...
    Glyph oe;
    for (LEdge otherArc : otherArcs) {
      broughtElements.add(BroughtElement.rememberPreviousState(otherArc));
      spline = otherArc.getSpline();
      elementsToFade.remove(spline);
      asp = spline.getStartPoint();
      aep = spline.getEndPoint();
      if (node2broughtPosition.containsKey(otherArc.getTail())
          && node2broughtPosition.containsKey(otherArc.getHead())) {
        sp = (Point2D.Double) node2broughtPosition.get(otherArc.getTail());
        ep = (Point2D.Double) node2broughtPosition.get(otherArc.getHead());
      } else {
        oe = otherArc.getOtherEnd(node).getShape();
        if (Math.sqrt((asp.x - ex) * (asp.x - ex) + (asp.y - ey) * (asp.y - ey)) <= Math
            .sqrt((aep.x - ex) * (aep.x - ex) + (aep.y - ey) * (aep.y - ey))) {
          sp = new Point2D.Double(bposition.x, bposition.y);
          ep = oe.getLocation();
        } else {
          sp = oe.getLocation();
  ...
  public void attemptLinkSliding(double press_vx, double press_vy, int scr_x, int scr_y) {
    ...
    if (lstruct != null) {
      ...
      double shortestDistance =
          Math.sqrt((nodeShape.vx - press_vx) * (nodeShape.vx - press_vx)
              + (nodeShape.vy - press_vy) * (nodeShape.vy - press_vy));
      double distance;
      ...
      if (shortestDistance < 2 * closestNode.getShape().getSize()) {
        ...
        shortestDistance =
            Math.sqrt((cPos.getX() - mPos.getX()) * (cPos.getX() - mPos.getX())
                + (cPos.getY() - mPos.getY()) * (cPos.getY() - mPos.getY()));
    ...
    // else select the edge hovered by the cursor (if any) - works even if no knowledge about
    // logical structure
    closestNode = null;
  ...
  public void startLinkSliding(final double press_vx, final double press_vy, int px, int py) {
    ...
    // add cursor on link
    slideCursor =
        new SICircle(press_vx, press_vy, 0, SLIDER_CURSOR_SIZE, SLIDER_CURSOR_FILL,
            ConfigManager.HIGHLIGHT_COLOR);
    slideCursor.setStroke(new BasicStroke(2f));
    // slideCursor.setFilled(false);
    mSpace.addGlyph(slideCursor);
    // display selection radius, circular zone that allows for arc switching when starting from a
    // node
    if (closestNode != null) {
      selectionRadius =
          new SICircle(closestNode.getShape().vx, closestNode.getShape().vy, 0, SELECTION_RADIUS,
    ...
    Animation a =
        animator.getAnimationFactory().createCameraTranslation(200, mainCamera,
            new Point2D.Double(press_vx, press_vy), false, SlowInSlowOutInterpolator.getInstance(),
            new EndAction() {
              @Override
  ...
  public void linkSlider(double vx, double vy, boolean centerCursor) {
    ...
    if (!withinSelectionRadius || centerCursor) {
      ...
      double shortestDistance =
          Math.sqrt((tPos.getX() - mtPos.getX()) * (tPos.getX() - mtPos.getX())
              + (cPos.getY() - mtPos.getY()) * (cPos.getY() - mtPos.getY()));
      double distance;
...
class Ring {
  ...
  // /* cones of influence, for each item, first element is the smallest angle in [0, 2Pi[, second
  // the largest angle in [0, 2Pi[ */
  ...
  boolean intersectsConeOfInfluence(double a1, double a2) {
    for (double[] cone : cones) {
      if (a2 > cone[0] && a1 < cone[1]) {
        return true;
      }
    }
    return false;
...
class DistanceComparator implements java.util.Comparator {
  ...
  public int compare(Object o1, Object o2) {
    ...
    double d1 =
        (centerShape.vx - n1.vx) * (centerShape.vx - n1.vx) + (centerShape.vy - n1.vy)
            * (centerShape.vy - n1.vy);
    double d2 =
        (centerShape.vx - n2.vx) * (centerShape.vx - n2.vx) + (centerShape.vy - n2.vy)
            * (centerShape.vy - n2.vy);
    if (d1 < d2) {
      return -1;
    } else if (d1 > d2) {
      return 1;
