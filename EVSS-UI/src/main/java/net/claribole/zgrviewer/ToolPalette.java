/*
 * FILE: ZGRViewer.java DATE OF CREATION: Wed Aug 30 12:02:31 2006 Copyright (c) INRIA, 2006-2011.
...
public class ToolPalette {
  ...
  boolean paintPalette = true; // set to false temporarily during panel resizing operations ; used
                               // as an optimization indicator
  ...
  void loadBuiltinModes() {
    ...
    while (i < BUILTIN_MODES.length) {
      if (modesToDisable.containsKey(BUILTIN_MODES[i])) {} else {
        buttons[j] =
            new VImage(0, -j * VERTICAL_STEP_BETWEEN_ICONS, 0, (new ImageIcon(this.getClass()
                .getResource(BUILTIN_MODE_ICON_PATHS.get(BUILTIN_MODES[i])))).getImage());
  ...
  /*
   * Called with false when resizing the main view to temporarily hide the palette until it actually
   * gets relocated to the top-left corner of that window. It is then called with true.
   */
  public void displayPalette(boolean b) {
    if (paintPalette == b) {
      return;
    }
    for (VImage button : buttons) {
      button.setVisible(b);
    }
    paintPalette = b;
  }
  ...
  public void updateHiddenPosition() {
    ...
    for (int i = 0; i < buttons.length; i++) {
      buttons[i].moveTo(wnes[0] - buttons[i].getWidth() / 2 + 1, wnes[1] - (i + 1)
          * VERTICAL_STEP_BETWEEN_ICONS);
    }
    displayPalette(true);
  }
    ...
    if (!visible) {
      ...
      Animation a =
          grMngr.vsm
              .getAnimationManager()
              .getAnimationFactory()
              .createCameraTranslation(ANIM_TIME, paletteCamera,
