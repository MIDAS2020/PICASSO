/*
 * FILE: GVLoader.java DATE OF CREATION: Mon Nov 27 08:30:31 2006 Copyright (c) INRIA, 2006-2011.
 * All Rights Reserved Licensed under the GNU LGPL. For full terms see the file COPYING.
 * 
...
class GVLoader {
  ...
  void open(short prg, boolean parser) {// prg is the program to use DOTManager.*_PROGRAM, use the
                                        // integrated parser or not
    if (ConfigManager.checkProgram(prg)) {
      openDOTFile(prg, parser);
    } else {
      Object[] options = {"Yes", "No"};
      int option =
          JOptionPane.showOptionDialog(null, ConfigManager.getDirStatus(), "Warning",
  ...
  void loadFile(File f, short prg, boolean parser) {
   // System.out.println("loadFile in");
    ...
    if (f.exists()) {
      ...
      //System.out.println("dotMngr.load in");
      dotMngr.load(f, prg, parser);
     // System.out.println("dotMngr.load out");
   ...
   // System.out.println("loadFile out");
  }
  ...
  void loadSVG(File f) {
    ...
    try {
      ...
      Document svgDoc =
          f.getName().toLowerCase().endsWith(".svgz") ? Utils.parse(new BufferedInputStream(
              new GZIPInputStream(new FileInputStream(f))), false) : Utils.parse(f, false);
  ...
  /**
   * Method used by ZGRViewer - Applet to get the server-side generated SVG file. Adds acceptance of
   * gzip encoding in request and handles response with gzip encoding. (i.e. SVGZ format).
   */
  void loadSVG(String svgFileURL) {
    try {
      ...
      if (svgDoc != null) {
        ...
        System.err.println("An error occurred while loading file " + svgFileURL);
      }
    } catch (Exception ex) {
      grMngr.reveal();
      ex.printStackTrace();
  ...
  void reloadFile() {
    // XXX: support integrated parser during reload
  ...
  // 12.5 new reloadFile called in SearchBox
  void reloadFile(File f) {
    // XXX: support integrated parser during reload
    if (cfgMngr.lastFileOpened != null) {
      grMngr.reset();
      this.loadFile(f, DOTManager.NEATO_PROGRAM, false);
    }
  }
}
