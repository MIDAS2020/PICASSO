/*
 * FILE: ZGRViewer.java DATE OF CREATION: Thu Jan 09 14:13:31 2003 Copyright (c) 2003 World Wide Web
 * Consortium. All Rights Reserved Copyright (c) INRIA, 2004-2011. All Rights Reserved Licensed
 * under the GNU LGPL. For full terms see the file COPYING.
 *
 * $Id: ZGRViewer.java 4961 2013-05-30 20:39:29Z epietrig $
 */
/*
 * Modified by: Jin Changjiu Modified Date: Dec.08,2009
 */
...
public class ZGRViewer implements ZGRApplication {
  ...
  static File cmdLineDOTFile = new File("graph.dot");
  ...
  private ExploreViewer exploreViewer;
  
  //we use this constructor
  public ZGRViewer(ChooseResultsInterface cr, int i) {
    initConfig();
    this.cr = cr;
    // init GUI after config as we load some GUI prefs from the config file
    initGUI(false, true);
    //JPanel subexploreview = this.getPanelView();
    //showExploreUI(subexploreview);
  }
  
   public ZGRViewer(ChooseResultsInterface cr,boolean explore) {
    if(explore==false) new ZGRViewer(cr) ;
    initConfig();
    this.cr = cr;
    // init GUI after config as we load some GUI prefs from the config file
    initGUI(false, true);
  ...
  public ZGRViewer(ChooseResultsInterface cr) {
    initConfig();
    this.cr = cr;
    // init GUI after config as we load some GUI prefs from the config file
    initGUI(false, false);
  ...
  public ZGRViewer(boolean flag1, boolean flag2) {
    initConfig();
    initGUI(flag1, flag2);
  }
  ...
  void initGUI(boolean acc, boolean viewOnJPanel) {
    ...
    if (viewOnJPanel) {
      _panelView = grMngr.createPanelView(grMngr.createZVTMelements(true), 500, 500);
      ...
      grMngr.createFrameView(grMngr.createZVTMelements(false), acc
          ? View.OPENGL_VIEW
          : View.STD_VIEW, jmb);
    }

    cfgMngr.notifyPlugins(Plugin.NOTIFY_PLUGIN_GUI_VIEW_CREATED);
    grMngr.parameterizeView(new ZgrvEvtHdlr(this, grMngr));
  ...
  JMenuBar initViewMenu(boolean accelerationMode) {
    ...
    openO.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Z, Toolkit.getDefaultToolkit()
        .getMenuShortcutKeyMask()));
    final JMenuItem openDG = new JMenuItem("SVG pipeline (standard)...");
    openDG.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_D, Toolkit.getDefaultToolkit()
        .getMenuShortcutKeyMask()));
    final JMenuItem openNG = new JMenuItem("SVG pipeline (standard)...");
    openNG.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_N, Toolkit.getDefaultToolkit()
        .getMenuShortcutKeyMask()));
    ...
    reloadI.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_R, Toolkit.getDefaultToolkit()
        .getMenuShortcutKeyMask()));
    ...
    printI.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_P, Toolkit.getDefaultToolkit()
        .getMenuShortcutKeyMask()));
    ...
    radarI.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_O, Toolkit.getDefaultToolkit()
        .getMenuShortcutKeyMask()));
    final JMenuItem showBkI = new JMenuItem("Show Bookmarks...");
    final JMenuItem searchI = new JMenuItem("Choose Results");
    searchI.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F, Toolkit.getDefaultToolkit()
        .getMenuShortcutKeyMask()));
    ...
    exitI.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Q, Toolkit.getDefaultToolkit()
        .getMenuShortcutKeyMask()));
  ...
  public void displayMainPieMenu(boolean b) {
    if (b) {
      ...
      mainPieMenu =
          PieMenuFactory.createPieMenu(Messages.mainMenuLabels, Messages.mainMenuLabelOffsets, 0,
    ...
    if (b) {
      ...
      if (index != -1) {
        ...
        if (label == Messages.PM_FILE) {
          subPieMenu =
              PieMenuFactory.createPieMenu(Messages.fileMenuLabels, Messages.fileMenuLabelOffsets,
                  0, grMngr.mainView, grMngr.vsm);
          items = subPieMenu.getItems();
          for (Glyph item : items) {
            item.setType(Messages.PM_ENTRY);
          }
        } else if (label == Messages.PM_EXPORT) {
          subPieMenu =
              PieMenuFactory.createPieMenu(Messages.exportMenuLabels,
                  Messages.exportMenuLabelOffsets, 0, grMngr.mainView, grMngr.vsm);
          items = subPieMenu.getItems();
          for (Glyph item : items) {
            item.setType(Messages.PM_ENTRY);
          }
  ...
  // open up the default or user-specified browser (netscape, ie,...) and try to display the content
  // uri
  ...
  void showExploreUI(JPanel subexploreview){
      if(exploreViewer==null){
          exploreViewer = new ExploreViewer(gvLdr, cr,subexploreview);
      }
      exploreViewer.setLocation(550, 50);
      exploreViewer.setVisible(true);
  }
  void showSearchBox() {
    if (searchBox == null) {
      searchBox = new SearchBox(gvLdr, cr);
    }
    searchBox.setLocation(550, 50);
      ...
      if (version != null) {
        if (version.equals(Messages.VERSION)) {
          ...
          JOptionPane.showMessageDialog(grMngr.mainView.getFrame(),
              Messages.YOU_HAVE_THE_MOST_RECENT_VERSION, "Version Information",
              JOptionPane.INFORMATION_MESSAGE);
        } else {
          JOptionPane.showMessageDialog(grMngr.mainView.getFrame(), Messages.NEW_VERSION_AVAILABLE
              + version + "\n" + Messages.DOWNLOAD_URL, "Version Information",
              JOptionPane.INFORMATION_MESSAGE);
        }
      } else {
        JOptionPane.showMessageDialog(grMngr.mainView.getFrame(),
            Messages.COULD_NOT_GET_VERSION_INFO, "Error", JOptionPane.ERROR_MESSAGE);
      }
    } catch (Exception ex) {
      JOptionPane.showMessageDialog(grMngr.mainView.getFrame(),
          Messages.COULD_NOT_GET_VERSION_INFO, "Error", JOptionPane.ERROR_MESSAGE);
  ...
  public void exit() {
    ...
    if (exitVMonClose()) {
      grMngr.mainView.setVisible(false);
      // System.exit(0);
    }
  }
  ...
  // added
  public void review() {
    grMngr.mainView.setVisible(true);
  }
    ...
    for (String arg : args) {
      if (arg.startsWith("-")) {
        if (arg.equals("--help") || arg.equals("-h")) {
          ...
          ConfigManager.plugInDir = new File(arg.substring(11));
        } else if (arg.startsWith("-pluginList=")) {
          ConfigManager.setPlugInJARs(arg.substring(12).split(","));
        } else if (arg.startsWith("-pluginMode=")) {
          ToolPalette.setDefaultPluginMode(arg.substring(12));
        }
      } else {
