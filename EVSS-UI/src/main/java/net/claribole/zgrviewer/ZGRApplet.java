/*
 * FILE: ZGRApplet.java DATE OF CREATION: Fri May 09 09:52:34 2003 Copyright (c) 2003 World Wide Web
...
public class ZGRApplet extends JApplet
  ...
  void initGUI() {
    ...
    viewPanel =
        grMngr.createPanelView(grMngr.createZVTMelements(true), appletWindowWidth,
            appletWindowHeight - 40);
    ...
    borderPanel.setBorder(BorderFactory.createTitledBorder(
        BorderFactory.createLineBorder(Color.black, 1), APPLET_TITLE));
  ...
  void displayURLinBrowser(String uri) {
    ...
    if (target == null) {
      target = ConfigManager._BLANK;
    }
    try {
      URL displayUrl;
      if (!(uri.startsWith(JAVASCRIPT_PROTOCOL) || uri.startsWith(HTTP_PROTOCOL)
          || uri.startsWith(HTTPS_PROTOCOL) || uri.startsWith(FTP_PROTOCOL) || uri
  ...
  public void keyPressed(KeyEvent e) {
    ...
    if (code == KeyEvent.VK_PAGE_UP) {
      ...
      if (grMngr.lensType != GraphicsManager.NO_LENS && grMngr.lens != null) {
        ...
        meh.tfactor =
            (grMngr.dmCamera.focal + Math.abs(grMngr.dmCamera.altitude)) / grMngr.dmCamera.focal;
        grMngr.dmCamera.altitudeOffset(-meh.tfactor * BaseEventHandler.WHEEL_ZOOMIN_FACTOR);
        grMngr.updateMagWindow();
        grMngr.vsm.repaint();
      } else {
        meh.tfactor =
            (grMngr.mainCamera.focal + Math.abs(grMngr.mainCamera.altitude))
                / grMngr.mainCamera.focal;
  ...
  static void buildConstraints(GridBagConstraints gbc, int gx, int gy, int gw, int gh, int wx,
      int wy) {
    gbc.gridx = gx;
    gbc.gridy = gy;
    gbc.gridwidth = gw;
...
class ZGRAGlassPane extends JComponent {
  ...
  static final AlphaComposite GLASS_ALPHA = AlphaComposite.getInstance(AlphaComposite.SRC_OVER,
      0.65f);
